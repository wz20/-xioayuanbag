...
Page({
    ...
    upload_img:function(){
      ...
      wx.chooseImage({
        ...
        success (res) {
          ...
          wx.cloud.uploadFile({
            cloudPath:'shangpin/'+timestamp+'.png',
    ...
    submit:function(e){
      let that=this
      console.log('e',e)
      if(e.detail.value.name!==""&&e.detail.value.price!==""&&e.detail.value.fenlei!==""&&e.detail.value.way!==""&&e.detail.value.tag!==""&&that.data.img.length!==0){
        db.collection('shangpin').add({
          data:{
            ...
            jianjie:e.detail.value.jianjie,
          },success:function(res){
            wx.showToast({
              title:'提交成功',
            })
